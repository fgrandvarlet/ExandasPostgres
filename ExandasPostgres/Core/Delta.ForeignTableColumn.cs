...
namespace ExandasPostgres.Core
{
    public partial class Delta
    {
        ...
        private void DeltaForeignTableColumn(FbConnection conn, List<DeltaReport> list)
        {
            ...
            sql = "SELECT s.foreign_table_name, s.column_name FROM src_foreign_table_column s" +
                " LEFT JOIN tgt_foreign_table_column t USING(foreign_table_name, column_name)" +
                " JOIN common_foreign_table USING(foreign_table_name)" +
                " WHERE t.column_name IS NULL" +
                " ORDER BY foreign_table_name, column_name";
            ...
            using (FbDataReader dr = cmd.ExecuteReader())
            {
                while (dr.Read())
                {
                    var report = new DeltaReport(this._schemaMapping.Uid, ENTITY, (string)dr["column_name"], (string)dr["foreign_table_name"], LabelId.ObjectInSourceNotInTarget);
                    list.Add(report);
                }
            }

            // phase 2 : target minus source
            sql = "SELECT t.foreign_table_name, t.column_name FROM tgt_foreign_table_column t" +
                " LEFT JOIN src_foreign_table_column s USING(foreign_table_name, column_name)" +
                " JOIN common_foreign_table USING(foreign_table_name)" +
            ...
            using (FbDataReader dr = cmd.ExecuteReader())
            {
                while (dr.Read())
                {
                    var report = new DeltaReport(this._schemaMapping.Uid, ENTITY, (string)dr["column_name"], (string)dr["foreign_table_name"], LabelId.ObjectInTargetNotInSource);
                    list.Add(report);
                }
            }

            // phase 3 : property differences between source and target
            sql = "SELECT * FROM comp_foreign_table_column";
            cmd = new FbCommand(sql, conn);
            ...
            using (FbDataReader dr = cmd.ExecuteReader())
            {
                while (dr.Read())
                {
                    var sourceForeignTableColumn = new ForeignTableColumn
                    {
                    ...
                    var targetForeignTableColumn = new ForeignTableColumn
                    {
                        RelationName = (string)dr["foreign_table_name"],
                        ColumnName = (string)dr["column_name"],
                    ...
                    sourceForeignTableColumn.Compare(targetForeignTableColumn, this._schemaMapping, list);
                }
            }
        }

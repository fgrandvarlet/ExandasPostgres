...
namespace ExandasPostgres.Core
{
    public partial class Delta
    {
        ...
        private void DeltaViewColumn(FbConnection conn, List<DeltaReport> list)
        {
            ...
            sql = "SELECT s.view_name, s.column_name FROM src_view_column s" +
                " LEFT JOIN tgt_view_column t USING(view_name, column_name)" +
                " JOIN common_view USING(view_name)" +
            ...
            using (FbDataReader dr = cmd.ExecuteReader())
            {
                while (dr.Read())
                {
                    var report = new DeltaReport(this._schemaMapping.Uid, ENTITY, (string)dr["column_name"], (string)dr["view_name"], LabelId.ObjectInSourceNotInTarget);
                    list.Add(report);
                }
            }

            // phase 2 : target minus source
            sql = "SELECT t.view_name, t.column_name FROM tgt_view_column t" +
                " LEFT JOIN src_view_column s USING(view_name, column_name)" +
                " JOIN common_view USING(view_name)" +
            ...
            using (FbDataReader dr = cmd.ExecuteReader())
            {
                while (dr.Read())
                {
                    var report = new DeltaReport(this._schemaMapping.Uid, ENTITY, (string)dr["column_name"], (string)dr["view_name"], LabelId.ObjectInTargetNotInSource);
                    list.Add(report);
                }
            }

            // phase 3 : property differences between source and target
            sql = "SELECT * FROM comp_view_column";
            cmd = new FbCommand(sql, conn);
            ...
            using (FbDataReader dr = cmd.ExecuteReader())
            {
                while (dr.Read())
                {
                    ...
                    sourceViewColumn.Compare(targetViewColumn, this._schemaMapping, list);
                }
            }
        }

...
namespace ExandasPostgres.Core
{
    public partial class Delta
    {
        ...
        private void DeltaPrimaryKey(FbConnection conn, List<DeltaReport> list)
        {
            ...
            sql = "SELECT s.table_name, s.constraint_name FROM src_primary_key s" +
                " LEFT JOIN tgt_primary_key t USING(table_name, constraint_name)" +
            ...
            sql = "SELECT t.table_name, t.constraint_name FROM tgt_primary_key t" +
                " LEFT JOIN src_primary_key s USING(table_name, constraint_name)" +
                " JOIN common_table USING(table_name)" +
            ...
            sql = "SELECT * FROM comp_primary_key";
            cmd = new FbCommand(sql, conn);
            ...
            using (FbDataReader dr = cmd.ExecuteReader())
            {
                while (dr.Read())
                {
                    ...
                    sourcePrimaryKey.Compare(targetPrimaryKey, this._schemaMapping, list);
                }
            }
        }
